...
public class EncryptionHandler implements MessageCryptoHandler {
  ...
  /**
   * Create ciphertext headers using the instance variables, and the provided content type and frame
   * size.
   *
   * @param contentType the content type to set in the ciphertext headers.
   * @param frameSize the frame size to set in the ciphertext headers.
   * @return the bytes containing the ciphertext headers.
   */
  private CiphertextHeaders createCiphertextHeaders(
      final ContentType contentType, final int frameSize) {
