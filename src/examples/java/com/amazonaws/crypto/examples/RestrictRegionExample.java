...
/**
 * Encrypts and then decrypts data using an AWS KMS customer master key in discovery mode. Discovery
 * mode is useful when you use an alias to identify a CMK when encrypting and the underlying key ARN
 * might vary in each AWS Region.
 *
 * <p>Arguments:
 *
 * <ol>
 *   <li>Key Name: An identifier for the AWS KMS customer master key (CMK) to use. For example, a
 *       key ARN or an alias name. See
 *       https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#key-id for a
 *       description of AWS KMS key identifiers.
 *   <li>Partition: The partition that the AWS KMS customer master key belongs in. This is usually
 *       "aws".
...
public class RestrictRegionExample {
  ...
  static void encryptAndDecrypt(
    ...
    //
    // In strict mode (`buildStrict`), the AWS KMS master key provider will only encrypt and decrypt
    // using the key
    ...
    final KmsMasterKeyProvider encryptingKeyProvider =
        KmsMasterKeyProvider.builder().buildStrict(keyName);
    ...
    // 6. Instantiate a Discovery Filter to restrict which AWS KMS CMKs an AWS KMS master key
    // provider
    // is allowed to attempt decryption with to a particular partition and account.
    // Configuring a Discovery AWS KMS master key provider with a Discovery Filter is a best
    // practice,
    ...
    // 7. Instantiate an AWS KMS master key provider for decryption in discovery mode
    // (`buildDiscovery`) with a
    ...
    final KmsMasterKeyProvider decryptingKeyProvider =
    ...
    final CryptoResult<byte[], KmsMasterKey> decryptResult =
        crypto.decryptData(decryptingKeyProvider, ciphertext);
