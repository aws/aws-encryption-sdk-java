 ...
 * </ol>
 *
 * You might use AWS Key Management Service (AWS KMS) for most encryption and decryption operations,
 * but still want the option of decrypting your data offline independently of AWS KMS. This sample
 ...
 *
 * <p>This sample uses the JCEMasterKey class to generate a RSA public-private key pair and saves
 * the key pair in memory. In practice, you would store the private key in a secure offline
...
public class EscrowedEncryptExample {
  ...
  private static void standardEncrypt(final String kmsArn, final String fileName) throws Exception {
    ...
    final JceMasterKey escrowPub =
        JceMasterKey.getInstance(
            publicEscrowKey, null, "Escrow", "Escrow", "RSA/ECB/OAEPWithSHA-512AndMGF1Padding");

    // 4. Combine the providers into a single master key provider
    final MasterKeyProvider<?> provider =
        MultipleProviderFactory.buildMultiProvider(keyProvider, escrowPub);
  ...
  private static void standardDecrypt(final String kmsArn, final String fileName) throws Exception {
    ...
    final CryptoOutputStream<?> decryptingStream =
        crypto.createDecryptingStream(provider, plaintextBuffer);
